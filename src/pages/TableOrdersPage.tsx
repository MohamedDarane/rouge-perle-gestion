...
import { Drink, TableOrderItem, TableOrder } from '../types';
import { createTableOrder, getDrinks, getTables } from '../services/cafeService';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { printTableOrder } from '../services/printerService';

const TableOrdersPage: React.FC = () => {
  const [cart, setCart] = useState<TableOrderItem[]>([]);
  const [drinks] = useState<Drink[]>(getDrinks());
  const [tables] = useState<number[]>(getTables());
  ...
  const submitOrder = () => {
    ...
    const order = createTableOrder(cart, selectedTable);
    
    if (order) {
      printTableOrder(order);
      setCart([]);
  ...
  return (
    <DashboardLayout>
      <div className="mb-6 text-center">
        <h1 className="text-3xl font-bold text-cafeBlack">Commandes de Table</h1>
              ...
              <>
                <div className="mb-4">
                  <Select
                    ...
                    onValueChange={(value) => setSelectedTable(value === "" ? null : parseInt(value))}
                  >
                    <SelectTrigger className="w-full border-cafeRed focus:ring-cafeRed">
